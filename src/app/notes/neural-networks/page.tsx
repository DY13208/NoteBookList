  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[96px] left-0 right-0 z-[110] pointer-events-none">
        <div className={`w-full max-w-md mx-auto bg-white border border-gray-200 rounded-t-3xl shadow-[0_-8px_30px_rgb(0,0,0,0.12)] transition-all duration-300 pointer-events-auto ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'}`}>
          <div
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="flex justify-between items-center py-3 px-6 cursor-pointer border-b border-gray-50"
          >
          ...
          <div className={`grid grid-cols-3 gap-4 pt-4 pb-8 px-6 transition-opacity duration-300 ${isAiExpanded ? 'opacity-100' : 'opacity-0'}`}>
