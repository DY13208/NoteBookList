  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[110] pointer-events-none">
        <div
          className={`w-full max-w-md mx-auto transition-all duration-300 pointer-events-auto bg-white border-t border-gray-200 shadow-[0_-4px_12px_rgba(0,0,0,0.05)] rounded-t-3xl fixed left-1/2 -translate-x-1/2 bottom-[96px] ${
            isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'
          }`}
        >
          {/* Toggle Header */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full pt-3 pb-2 px-6 flex flex-col items-center group"
          >
            <div className="w-12 h-1.5 bg-gray-200 rounded-full mb-3 group-hover:bg-gray-300 transition-colors"></div>
          ...
          <div className={`pt-2 pb-6 px-6 transition-opacity duration-300 ${isAiExpanded ? 'opacity-100' : 'opacity-0'}`}>
