...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-0 right-0 z-[110] pb-safe transition-all duration-500 bg-white border-t border-gray-200 shadow-[0_-4px_15px_rgba(0,0,0,0.1)] ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-40px)]'}`}>
        <div className="w-full max-w-md mx-auto px-6">
          <div
            className="flex justify-between items-center py-3 cursor-pointer group"
            onClick={() => setIsAiExpanded(!isAiExpanded)}
          >
            <h3 className="text-sm font-black text-s4-flat-primary uppercase tracking-widest flex items-center gap-2">
          ...
          <div className={`grid grid-cols-3 gap-4 pb-6 transition-all duration-500 ${isAiExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
