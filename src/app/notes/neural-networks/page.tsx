...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[100px] left-0 right-0 z-[110] transition-all duration-300 ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'}`}>
        <div className="w-full max-w-md mx-auto px-4">
          <div className="bg-white/95 backdrop-blur-md rounded-t-3xl border-2 border-black shadow-[0_-8px_30px_rgba(0,0,0,0.12)] overflow-hidden">
            {/* Toggle Handle */}
            <button
              onClick={() => setIsAiExpanded(!isAiExpanded)}
              className="w-full h-12 flex items-center justify-between px-6 border-b border-gray-100 hover:bg-gray-50 transition-colors"
            >
              <div className="flex items-center gap-2">
            ...
            <div className={`p-6 pt-4 transition-all duration-300 ${isAiExpanded ? 'opacity-100' : 'opacity-0'}`}>
              <div className="grid grid-cols-3 gap-4">
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-primary hover:bg-indigo-600 transition-all flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] active:translate-y-1 active:shadow-none">
                    <span className="material-symbols-outlined text-2xl">segment</span>
                  </div>
                  <span className="text-xs font-bold text-s4-flat-text">摘要</span>
                </button>
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-accent hover:bg-rose-600 transition-all flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] active:translate-y-1 active:shadow-none">
                    <span className="material-symbols-outlined text-2xl">lightbulb</span>
                  </div>
                  <span className="text-xs font-bold text-s4-flat-text">提取</span>
                </button>
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-yellow hover:bg-amber-600 transition-all flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] active:translate-y-1 active:shadow-none">
