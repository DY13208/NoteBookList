    ...
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[100px] left-0 right-0 z-[110] pointer-events-none">
        <div className="w-full max-w-md mx-auto px-4">
          <div className="bg-white dark:bg-gray-900 border-2 border-black dark:border-white rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] overflow-hidden pointer-events-auto transition-all duration-300">
            {/* Toggle Header */}
            <button
              onClick={() => setIsAiExpanded(!isAiExpanded)}
              className="w-full flex items-center justify-between px-5 py-3 bg-gray-50 dark:bg-white/5 border-b-2 border-black dark:border-white group"
            >
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-s4-flat-primary group-hover:animate-pulse">auto_awesome</span>
                <h3 className="text-sm font-black text-s4-flat-text dark:text-white uppercase tracking-widest">AI 操作</h3>
              </div>
            ...
            {/* Actions Grid */}
            <div className={`transition-all duration-300 overflow-hidden ${isAiExpanded ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
              <div className="grid grid-cols-3 gap-4 p-5">
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-primary border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] group-active:translate-y-0.5 group-active:shadow-none transition-all flex items-center justify-center text-white mb-2">
                    <span className="material-symbols-outlined text-2xl">segment</span>
                  </div>
                  <span className="text-[11px] font-black text-s4-flat-text dark:text-white uppercase tracking-tighter">摘要</span>
                </button>
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-accent border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] group-active:translate-y-0.5 group-active:shadow-none transition-all flex items-center justify-center text-white mb-2">
                    <span className="material-symbols-outlined text-2xl">lightbulb</span>
                  </div>
                  <span className="text-[11px] font-black text-s4-flat-text dark:text-white uppercase tracking-tighter">提取</span>
                </button>
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-yellow border-2 border-black dark:border-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] group-active:translate-y-0.5 group-active:shadow-none transition-all flex items-center justify-center text-white mb-2">
                    <span className="material-symbols-outlined text-2xl">quiz</span>
                  </div>
                  <span className="text-[11px] font-black text-s4-flat-text dark:text-white uppercase tracking-tighter">测验</span>
                </button>
              </div>
            </div>
          </div>
