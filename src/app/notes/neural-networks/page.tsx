...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
          ...
          <div className="bg-white space-y-8 relative">
            ...
            <div className="relative z-10 border-l-4 border-s4-flat-primary pl-4 py-1">
              <p className="text-s4-flat-text text-lg leading-relaxed font-normal">
                神经网络是一系列模拟<span className="bg-yellow-100 px-1 font-bold text-s4-flat-text">人脑</span>处理过程来识别数据中关系的算法。
              </p>
            </div>
      ...
      <div className="fixed bottom-[100px] left-0 right-0 z-[110] pointer-events-none transition-all duration-300">
        <div className="w-full max-w-md mx-auto pointer-events-auto">
          <div className="bg-white border-2 border-black rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.1)] overflow-hidden">
            {/* Handle/Header for Toggling */}
            <button
              onClick={() => setIsAiExpanded(!isAiExpanded)}
              className="w-full pt-4 pb-2 px-6 flex flex-col items-center gap-2 hover:bg-gray-50 transition-colors"
            >
              <div className="w-12 h-1.5 bg-gray-200 rounded-full"></div>
              <div className="flex justify-between items-center w-full">
                <h3 className="text-sm font-black text-s4-flat-text-light uppercase tracking-wider">AI 操作</h3>
            ...
            {/* Collapsible Content */}
            <div className={`px-6 pb-8 transition-all duration-300 ease-in-out ${isAiExpanded ? 'max-h-96 opacity-100 mt-4' : 'max-h-0 opacity-0 overflow-hidden'}`}>
              <div className="grid grid-cols-3 gap-4">
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-primary hover:bg-indigo-600 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center text-white mb-2">
                    <span className="material-symbols-outlined text-2xl">segment</span>
                  </div>
                  <span className="text-xs font-bold text-s4-flat-text">摘要</span>
                </button>
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-accent hover:bg-rose-600 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center text-white mb-2">
                    <span className="material-symbols-outlined text-2xl">lightbulb</span>
                  </div>
                  <span className="text-xs font-bold text-s4-flat-text">提取</span>
                </button>
                <button className="group flex flex-col items-center w-full">
                  <div className="w-full h-12 rounded-xl bg-s4-flat-yellow hover:bg-amber-600 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center text-white mb-2">
