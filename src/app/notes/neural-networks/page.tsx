...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-0 right-0 z-[110] pb-safe transition-all duration-300 bg-white border-t-2 border-s4-flat-text ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)] pointer-events-none'}`}>
        <div className="pointer-events-auto">
          <div className="w-full max-w-md mx-auto pt-2 pb-6 px-6">
            <div
              className="flex justify-between items-center mb-4 cursor-pointer py-2"
              onClick={() => setIsAiExpanded(!isAiExpanded)}
            >
              <h3 className="text-xs font-black text-s4-flat-text-light uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="grid grid-cols-3 gap-4">
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-primary border-2 border-s4-flat-text shadow-[4px_4px_0px_0px_#1e1b4b] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center text-white mb-2">
                  <span className="material-symbols-outlined text-2xl">segment</span>
                </div>
                <span className="text-xs font-bold text-s4-flat-text">摘要</span>
              </button>
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-accent border-2 border-s4-flat-text shadow-[4px_4px_0px_0px_#1e1b4b] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center text-white mb-2">
                  <span className="material-symbols-outlined text-2xl">lightbulb</span>
                </div>
                <span className="text-xs font-bold text-s4-flat-text">提取</span>
              </button>
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-yellow border-2 border-s4-flat-text shadow-[4px_4px_0px_0px_#1e1b4b] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center text-s4-flat-text mb-2">
