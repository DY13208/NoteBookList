...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-0 right-0 z-30 transition-all duration-300 ${isAiExpanded ? 'bg-white' : 'bg-transparent pointer-events-none'}`}>
        <div className="w-full max-w-md mx-auto relative">
          {/* Toggle Header */}
          <div
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="absolute -top-10 left-1/2 -translate-x-1/2 bg-white border-2 border-gray-200 border-b-0 px-4 py-1.5 rounded-t-2xl flex items-center gap-2 cursor-pointer shadow-sm pointer-events-auto active:scale-95 transition-transform"
          >
            <span className="material-symbols-outlined text-s4-flat-primary text-xl">auto_awesome</span>
            <span className="text-xs font-bold text-s4-flat-text-light uppercase tracking-widest">AI Actions</span>
          ...
          <div className={`bg-white border-t border-gray-200 pt-4 pb-6 px-6 transition-all duration-300 overflow-hidden ${isAiExpanded ? 'max-h-96 opacity-100 pointer-events-auto' : 'max-h-0 opacity-0 pointer-events-none'}`}>
