...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[100px] left-4 right-4 z-[110] transition-all duration-500 ease-in-out ${isAiExpanded ? 'translate-y-0 opacity-100' : 'translate-y-2'}`}>
        <div className="w-full max-w-md mx-auto bg-white/90 backdrop-blur-xl rounded-3xl border-2 border-gray-100 shadow-2xl overflow-hidden">
          {/* Collapse/Expand Toggle Header */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full px-6 py-3 flex items-center justify-between bg-gray-50/50 hover:bg-gray-50 transition-colors border-b border-gray-100"
          >
            <div className="flex items-center gap-2">
              <div className="size-2 rounded-full bg-s4-flat-primary animate-pulse"></div>
              <h3 className="text-[10px] font-black text-s4-flat-text-light uppercase tracking-[0.2em]">AI 操作</h3>
            </div>
            <span className={`material-symbols-outlined text-s4-flat-text-light transition-transform duration-300 ${isAiExpanded ? 'rotate-180' : ''}`}>
              keyboard_arrow_up
            </span>
          </button>
          ...
          <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isAiExpanded ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="p-6">
              <div className="grid grid-cols-3 gap-4">
                <button className="group flex flex-col items-center w-full transition-transform active:scale-95">
                  <div className="w-full h-14 rounded-2xl bg-s4-flat-primary hover:bg-indigo-600 transition-all flex items-center justify-center text-white mb-2 shadow-bold-sm group-hover:shadow-bold">
                    <span className="material-symbols-outlined text-2xl">auto_awesome</span>
                  </div>
                  <span className="text-[11px] font-bold text-s4-flat-text uppercase tracking-wider">摘要</span>
                </button>
                <button className="group flex flex-col items-center w-full transition-transform active:scale-95">
                  <div className="w-full h-14 rounded-2xl bg-s4-flat-accent hover:bg-rose-600 transition-all flex items-center justify-center text-white mb-2 shadow-bold-sm group-hover:shadow-bold">
                    <span className="material-symbols-outlined text-2xl">lightbulb</span>
                  </div>
                  <span className="text-[11px] font-bold text-s4-flat-text uppercase tracking-wider">提取</span>
                </button>
                <button className="group flex flex-col items-center w-full transition-transform active:scale-95">
                  <div className="w-full h-14 rounded-2xl bg-s4-flat-yellow hover:bg-amber-600 transition-all flex items-center justify-center text-white mb-2 shadow-bold-sm group-hover:shadow-bold">
                    <span className="material-symbols-outlined text-2xl">quiz</span>
                  </div>
                  <span className="text-[11px] font-bold text-s4-flat-text uppercase tracking-wider">测验</span>
                </button>
              </div>
            </div>
          </div>
