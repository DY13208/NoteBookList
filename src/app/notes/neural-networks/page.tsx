...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-0 right-0 z-[110] transition-all duration-300 pointer-events-none`}>
        <div className="w-full max-w-md mx-auto pointer-events-auto bg-white/95 backdrop-blur-md border-2 border-black rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.1)] overflow-hidden">
          <div
            className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 border-b border-gray-100 transition-colors"
            onClick={() => setIsAiExpanded(!isAiExpanded)}
          >
            <div className="flex items-center gap-2">
              <div className="size-8 rounded-lg bg-s4-flat-primary/10 flex items-center justify-center">
          ...
          <div className={`transition-all duration-300 ease-in-out ${isAiExpanded ? 'max-h-48 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
            <div className="grid grid-cols-3 gap-4 p-6">
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-primary hover:bg-indigo-600 transition-all flex items-center justify-center text-white mb-2 shadow-bold active:translate-y-1 active:shadow-none">
                  <span className="material-symbols-outlined text-2xl font-bold">segment</span>
                </div>
                <span className="text-xs font-black text-s4-flat-text">摘要</span>
              </button>
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-accent hover:bg-rose-600 transition-all flex items-center justify-center text-white mb-2 shadow-bold active:translate-y-1 active:shadow-none">
              ...
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-yellow hover:bg-amber-600 transition-all flex items-center justify-center text-white mb-2 shadow-bold active:translate-y-1 active:shadow-none">
                  <span className="material-symbols-outlined text-2xl font-bold">quiz</span>
                </div>
