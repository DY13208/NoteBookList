...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div
        className={`fixed left-0 right-0 z-[110] transition-all duration-300 bg-white border-t-2 border-gray-100 dark:border-white/10 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] bottom-[96px] ${
          isAiExpanded ? 'max-h-[300px]' : 'max-h-[48px]'
        } overflow-hidden`}
      >
        <div className="w-full max-w-md mx-auto px-6 pb-6">
          ...
          >
            <div className="w-12 h-1.5 rounded-full bg-gray-200 group-hover:bg-s4-flat-primary transition-colors"></div>
          </button>
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xs font-black text-s4-flat-text-light uppercase tracking-[0.2em]">AI 操作</h3>
            <span className="material-symbols-outlined text-s4-flat-primary animate-pulse text-[20px]">auto_awesome</span>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
              <div className="w-full h-12 rounded-xl bg-s4-flat-primary hover:bg-indigo-600 transition-colors flex items-center justify-center text-white mb-2 shadow-bold-sm border-2 border-black">
                <span className="material-symbols-outlined text-2xl">segment</span>
              </div>
              <span className="text-[11px] font-black text-s4-flat-text uppercase tracking-wider">摘要</span>
            </button>
            <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
              <div className="w-full h-12 rounded-xl bg-s4-flat-accent hover:bg-rose-600 transition-colors flex items-center justify-center text-white mb-2 shadow-bold-sm border-2 border-black">
                <span className="material-symbols-outlined text-2xl">lightbulb</span>
              </div>
              <span className="text-[11px] font-black text-s4-flat-text uppercase tracking-wider">提取</span>
            </button>
            <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
              <div className="w-full h-12 rounded-xl bg-s4-flat-yellow hover:bg-amber-600 transition-colors flex items-center justify-center text-white mb-2 shadow-bold-sm border-2 border-black">
                <span className="material-symbols-outlined text-2xl">quiz</span>
              </div>
              <span className="text-[11px] font-black text-s4-flat-text uppercase tracking-wider">测验</span>
            </button>
          </div>
        </div>
      </div>
