...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-0 right-0 z-[110] pb-safe transition-all duration-300 bg-white border-t-2 border-gray-100 ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'}`}>
        <div className="w-full max-w-md mx-auto px-6">
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full pt-4 pb-2 flex justify-between items-center group"
          >
            <h3 className="text-sm font-bold text-s4-flat-text-light uppercase tracking-wider group-hover:text-s4-flat-primary transition-colors">AI Actions</h3>
            <span className={`material-symbols-outlined text-gray-400 transition-transform duration-300 ${isAiExpanded ? 'rotate-180' : ''}`}>
              keyboard_arrow_up
            </span>
          </button>
          <div className={`grid grid-cols-3 gap-4 pb-6 transition-opacity duration-300 ${isAiExpanded ? 'opacity-100' : 'opacity-0'}`}>
