...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[100px] left-0 right-0 z-[110] pb-safe pointer-events-none">
        <div className="w-full max-w-md mx-auto bg-white border-2 border-black rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.1)] overflow-hidden pointer-events-auto">
          {/* Bar Header / Toggle */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full pt-4 pb-2 px-6 flex items-center justify-between hover:bg-gray-50 transition-colors border-b border-gray-100"
          >
            <h3 className="text-sm font-black text-s4-flat-text uppercase tracking-widest flex items-center gap-2">
          ...
          {/* Actions Content */}
          <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isAiExpanded ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="grid grid-cols-3 gap-4 pt-4 pb-6 px-6">
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-12 rounded-xl bg-s4-flat-primary border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center text-white mb-2">
              ...
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-12 rounded-xl bg-s4-flat-accent border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center text-white mb-2">
                  <span className="material-symbols-outlined text-2xl">lightbulb</span>
                </div>
                <span className="text-xs font-black text-s4-flat-text">提取</span>
              </button>
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-12 rounded-xl bg-s4-flat-yellow border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center text-white mb-2">
