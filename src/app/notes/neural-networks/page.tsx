    ...
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[100px] left-0 right-0 z-[110] pointer-events-none">
        <div className="w-full max-w-md mx-auto px-4">
          <div className={`bg-white dark:bg-[#1a0c11] rounded-2xl border-2 border-gray-100 dark:border-white/10 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] overflow-hidden transition-all duration-300 pointer-events-auto ${isAiExpanded ? 'max-h-96 opacity-100' : 'max-h-12 opacity-90'}`}>
            <button
              onClick={() => setIsAiExpanded(!isAiExpanded)}
              className="w-full flex items-center justify-between px-6 py-3 bg-gray-50/50 dark:bg-white/5 border-b border-gray-100 dark:border-white/5"
            >
              <div className="flex items-center gap-2">
            ...
            <div className={`p-6 transition-all duration-300 ${isAiExpanded ? 'opacity-100' : 'opacity-0 h-0 p-0 pointer-events-none'}`}>
