...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[100px] left-4 right-4 z-[110] transition-all duration-300 pointer-events-none">
        <div className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-md rounded-2xl border-2 border-gray-100 shadow-xl pointer-events-auto overflow-hidden">
          {/* Toggle Header */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full flex items-center justify-between px-6 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors"
          >
            <div className="flex items-center gap-2">
          ...
          {/* Actions Content */}
          <div className={`transition-all duration-300 ease-in-out ${isAiExpanded ? 'max-h-48 opacity-100 py-4 px-6' : 'max-h-0 opacity-0 overflow-hidden'}`}>
            <div className="grid grid-cols-3 gap-4">
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-primary/10 hover:bg-s4-flat-primary transition-colors flex items-center justify-center text-s4-flat-primary group-hover:text-white mb-2 border border-s4-flat-primary/20">
                  <span className="material-symbols-outlined text-2xl">segment</span>
                </div>
                <span className="text-[10px] font-bold text-s4-flat-text tracking-wider uppercase">摘要</span>
              </button>
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-accent/10 hover:bg-s4-flat-accent transition-colors flex items-center justify-center text-s4-flat-accent group-hover:text-white mb-2 border border-s4-flat-accent/20">
                  <span className="material-symbols-outlined text-2xl">lightbulb</span>
                </div>
                <span className="text-[10px] font-bold text-s4-flat-text tracking-wider uppercase">提取</span>
              </button>
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-12 rounded-xl bg-s4-flat-yellow/10 hover:bg-s4-flat-yellow transition-colors flex items-center justify-center text-s4-flat-yellow group-hover:text-white mb-2 border border-s4-flat-yellow/20">
                  <span className="material-symbols-outlined text-2xl">quiz</span>
                </div>
                <span className="text-[10px] font-bold text-s4-flat-text tracking-wider uppercase">测验</span>
              </button>
            </div>
          </div>
        </div>
