...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[96px] left-0 right-0 z-[110] transition-all duration-300 bg-white border-t-2 border-gray-100 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
        <div className="w-full max-w-md mx-auto">
          {/* Interactive Header Toggle */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors rounded-t-3xl"
          >
            <div className="flex items-center gap-2">
          ...
          {/* Collapsible Content */}
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isAiExpanded ? 'max-h-96 opacity-100 pb-8' : 'max-h-0 opacity-0'}`}>
            <div className="px-6">
