...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-0 right-0 z-30 pb-safe transition-all duration-500 bg-white border-t-2 border-gray-100 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'}`}>
        <div className="w-full max-w-md mx-auto">
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full pt-4 pb-2 px-6 flex justify-between items-center group"
          >
          ...
          <div className={`grid grid-cols-3 gap-4 px-6 pb-6 transition-all duration-300 ${isAiExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <button className="group flex flex-col items-center w-full">
              <div className="w-full h-12 rounded-lg bg-s4-flat-primary hover:bg-indigo-600 transition-colors flex items-center justify-center text-white mb-2 shadow-md active:translate-y-1 active:shadow-none">
                <span className="material-symbols-outlined text-2xl">segment</span>
              </div>
              <span className="text-xs font-bold text-s4-flat-text">摘要</span>
            </button>
            <button className="group flex flex-col items-center w-full">
              <div className="w-full h-12 rounded-lg bg-s4-flat-accent hover:bg-rose-600 transition-colors flex items-center justify-center text-white mb-2 shadow-md active:translate-y-1 active:shadow-none">
                <span className="material-symbols-outlined text-2xl">lightbulb</span>
              </div>
              <span className="text-xs font-bold text-s4-flat-text">提取</span>
            </button>
            <button className="group flex flex-col items-center w-full">
              <div className="w-full h-12 rounded-lg bg-s4-flat-yellow hover:bg-amber-600 transition-colors flex items-center justify-center text-white mb-2 shadow-md active:translate-y-1 active:shadow-none">
