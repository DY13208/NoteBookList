...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-4 right-4 z-[110] transition-all duration-500 ease-in-out ${isAiActionsExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'}`}>
        <div className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-md rounded-2xl border-2 border-gray-200 shadow-[0_-8px_30px_rgb(0,0,0,0.12)] overflow-hidden">
          {/* Toggle Header */}
          <button
            onClick={() => setIsAiActionsExpanded(!isAiActionsExpanded)}
            className="w-full pt-3 pb-2 px-6 flex items-center justify-between hover:bg-gray-50/50 transition-colors"
          >
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-s4-flat-primary text-xl">auto_awesome</span>
              <h3 className="text-xs font-black text-s4-flat-text-light uppercase tracking-[0.15em]">AI Actions</h3>
            </div>
            <span className={`material-symbols-outlined text-gray-400 transition-transform duration-300 ${isAiActionsExpanded ? 'rotate-180' : ''}`}>
              keyboard_arrow_up
            </span>
          </button>

          <div className={`px-6 pb-6 pt-2 transition-all duration-500 ${isAiActionsExpanded ? 'opacity-100 max-h-48' : 'opacity-0 max-h-0 pointer-events-none'}`}>
