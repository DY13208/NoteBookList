...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-30 pb-safe transition-all duration-500 bg-white border-t-2 border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.08)] ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-64px)]'}`}>
        <div className="w-full max-w-md mx-auto">
          {/* Handle/Toggle Area */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full h-16 flex items-center justify-between px-6 hover:bg-gray-50 transition-colors"
          >
            <div className="flex items-center gap-3">
              <div className="size-8 rounded-lg bg-s4-flat-primary flex items-center justify-center text-white shadow-sm">
                <span className="material-symbols-outlined text-xl">auto_awesome</span>
              </div>
              <h3 className="text-sm font-black text-s4-flat-text uppercase tracking-[0.1em]">AI 助手已就绪</h3>
            </div>
          ...
          <div className={`px-6 pb-10 transition-all duration-500 ${isAiExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <div className="grid grid-cols-3 gap-4">
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-14 rounded-2xl bg-s4-flat-primary hover:bg-indigo-600 transition-all flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] active:translate-y-0.5 active:shadow-none">
                  <span className="material-symbols-outlined text-2xl">segment</span>
                </div>
                <span className="text-xs font-bold text-s4-flat-text">AI 摘要</span>
              </button>
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-14 rounded-2xl bg-s4-flat-accent hover:bg-rose-600 transition-all flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] active:translate-y-0.5 active:shadow-none">
                  <span className="material-symbols-outlined text-2xl">lightbulb</span>
                </div>
                <span className="text-xs font-bold text-s4-flat-text">要点提取</span>
              </button>
              <button className="group flex flex-col items-center w-full">
                <div className="w-full h-14 rounded-2xl bg-s4-flat-yellow hover:bg-amber-600 transition-all flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] active:translate-y-0.5 active:shadow-none">
                  <span className="material-symbols-outlined text-2xl">quiz</span>
                </div>
                <span className="text-xs font-bold text-s4-flat-text">知识测验</span>
              </button>
            </div>
          </div>
        </div>
