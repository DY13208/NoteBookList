...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[96px] left-0 right-0 z-[110] pb-safe pointer-events-none">
        <div className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-md border-2 border-gray-200 rounded-t-3xl shadow-[0_-10px_25px_rgba(0,0,0,0.1)] pointer-events-auto overflow-hidden transition-all duration-500 ease-in-out">
          <div
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="flex justify-between items-center px-6 py-3 cursor-pointer hover:bg-gray-50 transition-colors border-b border-gray-100"
          >
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-s4-flat-primary animate-pulse">auto_awesome</span>
              <h3 className="text-sm font-black text-s4-flat-text tracking-wider uppercase">AI 操作</h3>
            </div>
          ...
          <div className={`transition-all duration-500 ease-in-out px-6 ${isAiExpanded ? 'max-h-96 pt-4 pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="grid grid-cols-3 gap-4">
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-14 rounded-2xl bg-s4-flat-primary hover:bg-indigo-600 transition-colors flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(79,70,229,0.3)]">
                  <span className="material-symbols-outlined text-2xl">segment</span>
                </div>
                <span className="text-xs font-bold text-s4-flat-text">摘要</span>
              </button>
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-14 rounded-2xl bg-s4-flat-accent hover:bg-rose-600 transition-colors flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(225,29,72,0.3)]">
                  <span className="material-symbols-outlined text-2xl">lightbulb</span>
                </div>
                <span className="text-xs font-bold text-s4-flat-text">提取</span>
              </button>
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-14 rounded-2xl bg-s4-flat-yellow hover:bg-amber-600 transition-colors flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(245,158,11,0.3)]">
