...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className="fixed bottom-[96px] left-0 right-0 z-30 pointer-events-none">
        <div className="w-full max-w-md mx-auto pointer-events-auto">
          <div className="bg-white/95 backdrop-blur-md border-2 border-gray-100 rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.08)] overflow-hidden transition-all duration-500 ease-in-out">
            <button
              onClick={() => setIsAiExpanded(!isAiExpanded)}
              className="w-full pt-4 pb-2 px-6 flex justify-between items-center group hover:bg-gray-50/50 transition-colors"
            >
              <div className="flex items-center gap-2">
                <div className="size-2 rounded-full bg-s4-flat-primary animate-pulse"></div>
                <h3 className="text-xs font-black text-s4-flat-text-light uppercase tracking-[0.15em]">AI 操作</h3>
              </div>
            ...
            <div className={`px-6 pb-6 overflow-hidden transition-all duration-500 ease-in-out ${isAiExpanded ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
              <div className="grid grid-cols-3 gap-4">
                <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                  <div className="w-full h-12 rounded-2xl bg-s4-flat-primary hover:bg-indigo-600 transition-colors flex items-center justify-center text-white mb-2 shadow-bold-sm">
                    <span className="material-symbols-outlined text-2xl">segment</span>
                  </div>
                  <span className="text-xs font-bold text-s4-flat-text">摘要</span>
                </button>
                <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                  <div className="w-full h-12 rounded-2xl bg-s4-flat-accent hover:bg-rose-600 transition-colors flex items-center justify-center text-white mb-2 shadow-bold-sm">
                    <span className="material-symbols-outlined text-2xl">lightbulb</span>
                  </div>
                  <span className="text-xs font-bold text-s4-flat-text">提取</span>
                </button>
                <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                  <div className="w-full h-12 rounded-2xl bg-s4-flat-yellow hover:bg-amber-600 transition-colors flex items-center justify-center text-white mb-2 shadow-bold-sm">
