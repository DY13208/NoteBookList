...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[100px] left-0 right-0 z-[110] transition-all duration-300 ${isAiExpanded ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        <div className="w-full max-w-md mx-auto px-4">
          <div className="bg-white/95 backdrop-blur-md border-2 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden pointer-events-auto">
            {/* Toggle Header */}
            <button
              onClick={() => setIsAiExpanded(!isAiExpanded)}
              className="w-full flex justify-between items-center bg-gray-50 px-4 py-3 border-b-2 border-black group"
            >
              <div className="flex items-center gap-2">
            ...
            {/* Actions Grid */}
            <div className={`transition-all duration-300 overflow-hidden ${isAiExpanded ? 'max-h-96 opacity-100 p-4' : 'max-h-0 opacity-0'}`}>
