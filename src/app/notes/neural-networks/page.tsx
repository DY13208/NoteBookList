...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary pb-24">
      {/* Header */}
      ...
      <div className="fixed bottom-[100px] left-0 right-0 z-[110] px-4 pointer-events-none">
        <div className="w-full max-w-md mx-auto bg-white dark:bg-[#1a0c11] rounded-2xl border-2 border-gray-100 dark:border-white/10 shadow-2xl pointer-events-auto overflow-hidden transition-all duration-300">
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full pt-4 pb-2 px-6 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
          >
          ...
          <div className={`grid grid-cols-3 gap-4 px-6 pb-6 transition-all duration-300 ${isAiExpanded ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0 pointer-events-none'}`}>
