...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[96px] left-0 right-0 z-[110] transition-all duration-300 bg-white border-t-2 border-gray-100 shadow-[0_-10px_30px_rgba(0,0,0,0.1)] rounded-t-3xl ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'}`}>
        <div className="w-full max-w-md mx-auto pt-2 pb-6 px-6">
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full flex justify-center py-2 mb-2 group"
          >
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-black text-s4-flat-text uppercase tracking-widest">AI Actions</h3>
            </div>
            <span className="text-[10px] font-bold bg-s4-flat-primary/10 text-s4-flat-primary px-2 py-0.5 rounded-full border border-s4-flat-primary/20">Beta</span>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <button className="group flex flex-col items-center w-full">
              <div className="w-full h-14 rounded-2xl bg-s4-flat-primary hover:bg-indigo-600 transition-all flex items-center justify-center text-white mb-2 shadow-[0_4px_0_0_#4338ca] active:translate-y-1 active:shadow-none">
                <span className="material-symbols-outlined text-2xl">segment</span>
              </div>
              <span className="text-[10px] font-black text-s4-flat-text uppercase tracking-tight">Summarize</span>
            </button>
            <button className="group flex flex-col items-center w-full">
              <div className="w-full h-14 rounded-2xl bg-s4-flat-accent hover:bg-rose-600 transition-all flex items-center justify-center text-white mb-2 shadow-[0_4px_0_0_#be123c] active:translate-y-1 active:shadow-none">
                <span className="material-symbols-outlined text-2xl">lightbulb</span>
              </div>
              <span className="text-[10px] font-black text-s4-flat-text uppercase tracking-tight">Extract</span>
            </button>
            <button className="group flex flex-col items-center w-full">
              <div className="w-full h-14 rounded-2xl bg-s4-flat-yellow hover:bg-amber-600 transition-all flex items-center justify-center text-white mb-2 shadow-[0_4px_0_0_#b45309] active:translate-y-1 active:shadow-none">
                <span className="material-symbols-outlined text-2xl">quiz</span>
              </div>
              <span className="text-[10px] font-black text-s4-flat-text uppercase tracking-tight">Quiz Me</span>
            </button>
          </div>
        </div>
      </div>
