...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div
        className={`fixed left-0 right-0 z-[110] transition-transform duration-300 bg-white border-t border-gray-200 bottom-[96px] ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%+2px)]'}`}
      >
        <div className="w-full max-w-md mx-auto pt-4 pb-6 px-6 relative">
          {/* Toggle Header */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="absolute -top-10 right-6 bg-white border-t-2 border-x-2 border-gray-200 rounded-t-xl px-4 py-2 flex items-center gap-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]"
          >
            <span className="material-symbols-outlined text-s4-flat-primary">
              {isAiExpanded ? 'keyboard_arrow_down' : 'keyboard_arrow_up'}
            </span>
            <span className="text-xs font-bold text-s4-flat-text uppercase tracking-widest">AI Actions</span>
          </button>
