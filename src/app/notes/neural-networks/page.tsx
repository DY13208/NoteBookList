...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div
        className={`fixed bottom-[100px] left-4 right-4 z-[110] transition-all duration-300 ${
          isAiExpanded ? 'bg-white' : 'bg-transparent'
        }`}
      >
        <div className={`w-full max-w-md mx-auto rounded-2xl border-2 border-black dark:border-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] overflow-hidden transition-all duration-300 ${isAiExpanded ? 'bg-white' : 'bg-transparent border-none shadow-none'}`}>
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className={`w-full flex items-center justify-between px-4 py-3 bg-s1-flat-mint text-black border-b-2 border-black transition-all ${!isAiExpanded ? 'rounded-xl border-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]' : ''}`}
          >
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined">auto_awesome</span>
              <span className="text-sm font-bold uppercase tracking-wider">AI 操作</span>
            </div>
            <span className="material-symbols-outlined transition-transform duration-300" style={{ transform: isAiExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>
              expand_less
            </span>
          </button>
