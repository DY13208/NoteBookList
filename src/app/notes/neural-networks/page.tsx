...
export default function AINotebook() {
  ...
  return (
    <div className="text-s4-flat-text antialiased overflow-hidden h-screen flex flex-col font-sans bg-s4-flat-bg selection:bg-s4-flat-primary/20 selection:text-s4-flat-primary">
      ...
      <div className={`fixed bottom-[100px] left-0 right-0 z-[110] transition-all duration-500 ease-in-out ${isAiExpanded ? 'translate-y-0' : 'translate-y-[calc(100%-48px)]'}`}>
        <div className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-md border-2 border-black dark:border-white rounded-t-3xl shadow-[0_-10px_30px_rgba(0,0,0,0.1)] overflow-hidden">
          {/* Toggle Header */}
          <button
            onClick={() => setIsAiExpanded(!isAiExpanded)}
            className="w-full py-3 px-6 flex items-center justify-between border-b border-gray-100 hover:bg-gray-50 transition-colors"
          >
            <div className="flex items-center gap-2">
              <div className="size-2 rounded-full bg-s4-flat-primary animate-pulse"></div>
              <h3 className="text-xs font-black text-s4-flat-text uppercase tracking-[0.2em]">AI 操作</h3>
            </div>
          ...
          <div className={`p-6 pt-4 transition-all duration-500 ${isAiExpanded ? 'opacity-100' : 'opacity-0 h-0 p-0 overflow-hidden'}`}>
            <div className="grid grid-cols-3 gap-4">
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-12 rounded-xl bg-s4-flat-primary hover:bg-indigo-600 transition-colors flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] border-2 border-black group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:shadow-none transition-all">
                  <span className="material-symbols-outlined text-2xl">segment</span>
                </div>
                <span className="text-[10px] font-bold text-s4-flat-text uppercase tracking-wider">摘要</span>
              </button>
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-12 rounded-xl bg-s4-flat-accent hover:bg-rose-600 transition-colors flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] border-2 border-black group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:shadow-none transition-all">
                  <span className="material-symbols-outlined text-2xl">lightbulb</span>
                </div>
                <span className="text-[10px] font-bold text-s4-flat-text uppercase tracking-wider">提取</span>
              </button>
              <button className="group flex flex-col items-center w-full active:scale-95 transition-transform">
                <div className="w-full h-12 rounded-xl bg-s4-flat-yellow hover:bg-amber-600 transition-colors flex items-center justify-center text-white mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] border-2 border-black group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:shadow-none transition-all">
                  <span className="material-symbols-outlined text-2xl">quiz</span>
                </div>
                <span className="text-[10px] font-bold text-s4-flat-text uppercase tracking-wider">测验</span>
              </button>
            </div>
          </div>
        </div>
