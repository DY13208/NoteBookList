  ...
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden max-w-md mx-auto bg-signup-bg dark:bg-background-dark font-jakarta text-text-main dark:text-white transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <p className="text-signup-text-muted dark:text-gray-400 text-center px-6 pb-6 text-sm font-medium">
          创建您的账户，开始与AI伙伴一起学习。
        </p>

        <form className="flex flex-col px-4 gap-4 pb-8" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              />
              <button
                className="absolute right-4 top-1/2 -translate-y-1/2 text-signup-text-muted dark:text-gray-500 hover:text-text-main dark:hover:text-white"
                type="button"
              >
          ...
          <div className="flex items-start gap-3 mt-2 px-1">
            <div className="relative flex items-center">
              ...
              />
              <span
                className="material-symbols-outlined absolute pointer-events-none opacity-0 peer-checked:opacity-100 text-text-main inset-0 m-auto"
                style={{ fontSize: "18px" }}
              >
                check
              </span>
            ...
            <label
              className="text-sm font-medium text-signup-text-muted dark:text-gray-400 leading-snug cursor-pointer select-none"
              htmlFor="terms"
            >
              我同意
              <a
                className="text-text-main dark:text-white font-bold underline decoration-2 decoration-signup-primary/50 underline-offset-2 hover:decoration-signup-primary"
                href="#"
              >
                服务条款
              </a>
          ...
          <button
            className="mt-2 w-full h-14 rounded-xl bg-signup-primary hover:bg-signup-primary/90 text-text-main font-extrabold text-lg tracking-wide transition-all active:scale-[0.98] shadow-sm flex items-center justify-center gap-2 disabled:opacity-60"
            type="submit"
            disabled={loading}
          >
            <span>{loading ? "注册中..." : "加入冒险"}</span>
        ...
        <div className="pb-8 pt-2 text-center">
          <p className="text-signup-text-muted dark:text-gray-400 text-sm font-medium">
            已有账号？
            <Link className="text-text-main dark:text-white font-bold hover:text-signup-primary transition-colors ml-1" href="/login">
              立即登录
            </Link>
          </p>
        </div>
